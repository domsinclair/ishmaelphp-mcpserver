<?php
    ...
    final class ProjectRootLocator
    {
        ...
        {
            // New: allow explicit override via environment
            $override = getenv('ISH_PROJECT_ROOT');
            if (is_string($override) && $override !== '') {
                $ov = $this->normalizePath($override);
                if ($this->looksLikeIshmaelProject($ov)) {
                    return $ov;
                }
            }

            $dir = $startDir !== null
                ? $this->normalizePath($startDir)
                : $this->normalizePath(getcwd() ?: '.');
        ...
        private function looksLikeIshmaelProject(string $dir): bool
        {
            ...
            if (is_file($composer)) {
                ...
                if (is_array($data)) {
                    ...
                    foreach ($all as $pkg) {
                        ...
                        $p = strtolower($pkg);
                        if (str_starts_with($p, 'ishmaelphp/') || str_starts_with($p, 'ishmael/')) {
                            return true;
                        }
                        if ($p === 'ishmael/framework' || $p === 'ishmael/mcp-server') {
                            return true;
                        }
                    }
                }
            ...
            $fileMarker = $dir . DIRECTORY_SEPARATOR . 'IshmaelPHP-Core' . DIRECTORY_SEPARATOR . 'bootstrap';
            if (is_file($fileMarker)) {
                return true;
            }
            $dirMarker = $dir . DIRECTORY_SEPARATOR . 'IshmaelPHP-Core';
            if (is_dir($dirMarker)) {
                return true;
            }
