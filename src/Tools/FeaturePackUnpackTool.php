    ...
    final class FeaturePackUnpackTool implements Tool
    {
        ...
        public function execute(array $input): array
        {
            ...
            for ($i = 0; $i < $zip2->numFiles; $i++) {
                $name = str_replace('\\', '/', $zip2->getNameIndex($i));
                ...
                $stream = $zip2->getStream($zip2->getNameIndex($i));
                if ($stream === false) {
                    // Try original name if normalized failed
                    $originalName = $zip2->getNameIndex($i);
                    $stream = $zip2->getStream($originalName);
                }
                if ($stream !== false) {
        ...
        public function inspect(ZipArchive $zip): array
        {
            ...
            foreach ($names as $n) {
                $norm = str_replace('\\', '/', $n);
                $namesNormalized[] = $norm;
                $parts = explode('/', $norm, 2);
                if ($parts[0] !== '') {
                    $topDirs[$parts[0]] = true;
                }
            }
            $names = $namesNormalized;

            $prefix     = '';
            $moduleName = '';

            // Check for specific Modules/ structure
            if (isset($topDirs['Modules'])) {
                $subDirs = [];
                foreach ($names as $n) {
                    if (str_starts_with($n, 'Modules/') && $n !== 'Modules/') {
                        $innerParts = explode('/', substr($n, 8), 2);
                        if ($innerParts[0] !== '') {
                            $subDirs[$innerParts[0]] = true;
                        }
                    }
                }

                if (count($subDirs) === 1) {
                    $candidate = array_key_first($subDirs);
                    $prefix     = 'Modules/' . $candidate . '/';
                    $moduleName = $candidate;
                }
            }

            if ($prefix === '' && count($topDirs) === 1) {
            ...
            if (!in_array($composerJson, $names, true)) {
                ...
                if ($content === false) {
                    // Try again with the normalized name if getFromName failed
                    $content = $zip->getFromName(str_replace('/', '\\', $composerJson));
                }
